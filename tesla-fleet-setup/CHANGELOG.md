...
- Update aiohttp to 3.14.3 to address known vulnerabilities found during release checks.

### Added

- Separate "Start over with a new key" action for a fresh Tesla account or developer app setup. Existing keys are backed up in a private directory before replacement.

### Fixed

- Explain duplicate public-key registration errors and clarify that the default reset preserves the signing key.
- Clear the selected Fleet API region when resetting setup.

## [0.9.5] - 2026-03-17

### Added

- **OAuth consent screenshot** — updated screenshot showing Tesla's permission
  screen with all scopes, highlights the Allow button
- **Vehicle Location warning** — amber warning box in step 5 reminding users
  that skipping the Vehicle Location permission breaks live location tracking

### Removed

- Old `select-api-options.png` screenshot replaced with full-page consent screen

## [0.9.4] - 2026-03-12

### Fixed

- **Partner registration now uses selected region only** — previously registered
  on both NA and EU endpoints which could cause "Region is invalid" errors in
  the HA Tesla Fleet integration, breaking live location data
- Region dropdown tip updated to warn that wrong region prevents location data

### Changed

- `register_partner` accepts explicit `api_base` parameter instead of always
  trying both endpoints
- Registration success message now shows which region was registered

## [0.9.3] - 2026-03-12

### Added

- "Use Cloudflare Tunnel instead" link in guided mode — no need to wait for
  DuckDNS failure to access the Cloudflare flow
...
- Add missing `vehicle_location` and `user_data` OAuth scopes — Tesla
  requires separate `vehicle_location` scope for location data access,
  was causing 403 "missing scopes vehicle_location" on all vehicle data

## [0.8.7] - 2026-03-11

### Added

- Re-authorize button in Test API Connection section to refresh OAuth token
- Log actual 403 error response body for debugging scope issues

## [0.8.6] - 2026-03-11

### Changed

- Added note to Test API Connection section explaining 403 errors are normal
  if HA integration works — wizard uses its own OAuth token with potentially
  different scopes
...
- Vehicle data API tests now return full data (charge_state, climate_state,
  drive_state, vehicle_state) — endpoints parameter needed URL-encoded
  semicolons per Tesla Fleet API spec, was using commas

## [0.8.4] - 2026-03-11

### Fixed

- Signing proxy auto-starts when completion page loads if it's not already
  running (was only auto-starting on add-on boot, not after OAuth completion
  via the polling path)

## [0.8.3] - 2026-03-11

### Changed

- Cloudflare tunnel setup steps now use the same spinning/tick/cross icons
  as the reachability test (replaced emoji icons)
- Reuse existing Cloudflare tunnel if one with the same name exists instead
  of failing — checks for existing tunnel before attempting to create

## [0.8.2] - 2026-03-11

### Added

- Auto-inject credentials into HA on completion page load (retry if first
  attempt during OAuth callback failed)
- Manual credential fallback on completion page — expandable section shows
  client ID and secret with copy buttons for pasting into HA if auto-inject
  doesn't work
...
- Auto-detect OAuth completion — wizard polls /api/status every 3 seconds
  after opening the Tesla sign-in window and automatically advances to the
  completion page when tokens are received (no manual refresh needed)

## [0.8.0] - 2026-03-11

### Changed

- Reachability test now checks the actual public URL (not localhost:443) —
  properly detects double NAT, ISP port blocking, and DNS issues
- Auto-runs reachability test after DuckDNS activation completes
- On failure, shows a prominent warning banner and scrolls to Cloudflare
  Tunnel section as the recommended alternative

## [0.7.9] - 2026-03-10

### Fixed

- Auto-populate Step 3 URLs (origin, redirect) after Cloudflare tunnel setup —
  publicKeyUrl was not being set on the Cloudflare success path

## [0.7.8] - 2026-03-10

### Fixed

- Fix cloudflared command-line flags order (`--no-autoupdate` is a global flag)
- Step-by-step progress UI for Cloudflare tunnel setup with green ticks
- Each step (verify, create tunnel, configure, DNS, start) shows individual status

## [0.7.7] - 2026-03-10

### Fixed

- Added cloudflared version check, debug logging, and protocol flag to
  diagnose silent tunnel startup failures
- Log tunnel token metadata (length/type) to verify API response format

## [0.7.6] - 2026-03-10

### Fixed

- Capture cloudflared stdout+stderr together so tunnel startup errors are
  visible in the log (was capturing stderr only, but cloudflared writes to stdout)
- Increased tunnel startup wait from 3s to 5s

## [0.7.5] - 2026-03-10

### Fixed

- Cloudflare tunnel creation "Json deserialize error" — tunnel secret was
  using URL-safe base64 (with underscores) instead of standard base64
...
- Cloudflare token instructions now include Zone → Zone → Read permission
  (required for the add-on to list your domains after verifying the token)
